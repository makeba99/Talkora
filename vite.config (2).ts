...
export default defineConfig({
  ...
  server: {
    allowedHosts: true,
    // Prevent browsers from caching Vite dev responses.
    // Without this, when Vite re-optimizes deps (new browserHash), browsers
    // keep stale source-file chunks that still import react@v=<old-hash>
    // while the main bundle fetches react@v=<new-hash> — two React instances
    // → "Cannot read properties of null (reading 'useState')".
    // In dev mode, freshness always beats cache efficiency.
    headers: {
