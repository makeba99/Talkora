...
| `HF_TOKEN` | Hugging Face **Classic Read** token (`hf_...`) for Sesame CSM-1B. Accept the license at huggingface.co/sesame/csm-1b. Fine-grained tokens usually cannot run inference. |
| `FAL_KEY` | Optional but recommended. fal.ai API key for **fal-ai/csm-1b** (real GPU Sesame audio). |
| `AI_VOICE_PROVIDER` | `edge` (default), `openai`, `browser`, or `sesame` |
| `AI_VOICE_SESAME_SPACE` | Optional custom Space id (must request ≤60s GPU). Default official Space cannot run from Railway. |
...
Sesame CSM-1B does **not** use the public ZeroGPU Space from Railway (that Space asks for 180 seconds per `/infer` and free Hugging Face accounts cannot grant it). Set `AI_VOICE_PROVIDER=sesame` plus:

1. `HF_TOKEN` — Classic Read token, and accept the gated license at `https://huggingface.co/sesame/csm-1b`
2. `FAL_KEY` — fal.ai key for `fal-ai/csm-1b` (this is the reliable GPU path)

In-room tutors fall back to the on-device browser voice only if both hosts fail.
