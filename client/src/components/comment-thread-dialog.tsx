      ...
      >
              ...
              return (
                <div key={c.id} className="flex gap-2.5 group" data-testid={`comment-${c.id}`}>
                  <div className="flex-shrink-0 w-8 h-8 rounded-full overflow-hidden bg-gradient-to-br from-cyan-400/30 to-violet-400/30 flex items-center justify-center text-xs font-bold text-white border border-white/10">
                    {avatar ? (
                      <img loading="lazy" decoding="async" src={avatar} alt={name} width={32} height={32} className="w-full h-full object-cover" />
                    ) : (
                      getInitials(name)
                    )}
                  </div>
        ...
        >
          {user?.profileImageUrl ? (
            <img loading="lazy" decoding="async" src={user.profileImageUrl} alt="" width={28} height={28} className="w-7 h-7 rounded-full object-cover flex-shrink-0 border border-white/15" />
          ) : (
