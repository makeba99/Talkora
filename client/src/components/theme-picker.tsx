...
export function ThemePicker({ open: controlledOpen, onOpenChange, hideTrigger }: ThemePickerProps = {}) {
  ...
  // When used without a real trigger (controlled externally from orbit/header pin),
  // render as a Dialog so Radix has a proper modal anchor and doesn't scroll/misplace.
  if (hideTrigger || isControlled) {
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          className="max-w-sm w-full max-h-[85vh] overflow-y-auto p-4"
          data-testid="dialog-theme-picker"
        >
          ...
          <ThemePickerInner open={open} setOpen={setOpen} />
        </DialogContent>
      </Dialog>
    );
  }

  // Standalone mode — renders its own trigger button + popover
  return (
