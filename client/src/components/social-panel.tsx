              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    role="switch"
                    aria-checked={roomJoinOn}
                    aria-pressed={roomJoinOn}
                    data-testid={`toggle-notif-room-follower-${u.id}`}
                    className={`relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-all duration-200 focus:outline-none select-none ${roomJoinOn ? "bg-emerald-500 shadow-[0_0_10px_rgba(52,211,153,0.75)]" : "bg-muted-foreground/25"}`}
                  >
                    <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-md transition-transform duration-200 ease-in-out ${roomJoinOn ? "translate-x-4" : "translate-x-0"}`} />
                  </button>
                </div>
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-all duration-200 focus:outline-none select-none ${dmOn ? "bg-emerald-500 shadow-[0_0_10px_rgba(52,211,153,0.75)]" : "bg-muted-foreground/25"}`}
                  >
                    <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-md transition-transform duration-200 ease-in-out ${dmOn ? "translate-x-4" : "translate-x-0"}`} />
                  </button>
                </div>
              </div>
            </PopoverContent>
