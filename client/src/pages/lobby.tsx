      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden app-scrollbar" id="main-content">
        <div className="max-w-[1600px] mx-auto p-3 sm:p-4 lg:px-6 xl:px-8 pb-0 space-y-5 animate-fade-in min-w-0">
          ...
          {activeDiscovery !== "rooms" ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-2 gap-y-0 lg:gap-y-0 xl:gap-y-0.5">
              {(() => {
                ...
                return idx === 0 ? (
                <div key={room.id} data-tour-target="rooms" style={{ contain: "layout paint", minHeight: 300 }}>
                    {card}
                  </div>
                ) : (
                  <div key={room.id} style={{ contain: "layout paint", minHeight: 300 }}>{card}</div>
                );
                });
              })()}
            </div>
