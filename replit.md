...
## 2026-05 Score 92+ Target Optimizations (Unused JS, TBT, critical bundle reduction)

Four targeted changes — zero design, layout, animation, or functionality impact:

| # | Change | File | Issue addressed |
|---|--------|------|-----------------|
| J | Added `@radix-ui/react-dialog`, `@radix-ui/react-dropdown-menu`, `@radix-ui/react-select`, `@radix-ui/react-separator`, `@radix-ui/react-label`, `@radix-ui/react-scroll-area` to `RADIX_DEFERRED` | `vite.config.ts` | **Unused JS / TBT**: These 6 Radix packages (~40–50 KB gzipped) were in `react-vendor` (the first chunk downloaded on every page load). None are imported by `lobby.tsx` or `room-card.tsx`. Moving them to `radix-deferred` shrinks `react-vendor` significantly — fewer bytes parsed on first paint → lower TBT. ES module static-import chain still guarantees `react-vendor` evaluates before `radix-deferred` (no forwardRef race possible). |
| K | Removed `"/components/ui/select"` from `LOBBY_CRITICAL_UI` | `vite.config.ts` | **Unused JS**: `select.tsx` (wrapping `@radix-ui/react-select`) was forced into `ui-components` despite neither `lobby.tsx` nor `room-card.tsx` importing it. Removing it lets Rollup co-locate it with `CreateRoomDialog`/`RoomEditDialog` lazy chunks where it's actually needed, saving ~12 KB from the critical `ui-components` chunk. |
| L | Removed `update-available-toast` and `pwa-install-banner` from Lucide icon `LOBBY_PATHS` | `vite.config.ts` | **Unused JS**: Both components are lazy (DeferredToasts 4 s delay, DeferredOverlays 8 s delay). Their exclusive Lucide icons (`RefreshCw`, `Share`, `PlusSquare`, `MoreVertical`, `Download`) were being forced into the critical `icons-vendor` chunk. Now they travel with their lazy consumer chunks and are never downloaded on first paint. |
| M | Link header script preload slots bumped from 6 → 8 | `server/static.ts` | **LCP / TBT**: The `app-constants` (rank 6) and `room-card` (rank 7) named chunks were matched by `criticalScriptPatterns` but excluded by the old `slice(0, 6)` limit. Raising to `slice(0, 8)` ensures all 8 lobby-critical chunks appear in the HTTP `Link:` Early Hints header, starting their download before HTML parsing begins. Total header size remains ~1.5 KB (within 8 KB limit). |

## 2026-05 Score 92+ Optimizations — Critical Chain + react-vendor Size

Two targeted changes addressing the "Avoid chaining critical requests" and "Reduce unused JavaScript" audits — zero design, layout, animation, or functionality impact:

| # | Change | File | Issue addressed |
|---|--------|------|-----------------|
| N | Added Vite entry `index-*.js` to `criticalScriptPatterns` at rank −2 (highest priority) | `server/static.ts` | **Critical chain**: The entry module (`index-BLYOn_QW.js`, 4.23 KiB gzipped) was discovered only during HTML body parsing (~411 ms), adding 602 ms of sequential latency. Adding it to the Link header means the browser starts fetching it via 103 Early Hints before HTML parsing begins — the sequential chain is eliminated. Slice limit bumped 8 → 10 to accommodate. |
| O | Split `@floating-ui/core`, `@floating-ui/dom`, `@floating-ui/utils` into new `floating-vendor` chunk | `vite.config.ts` + `server/static.ts` | **Unused JS / TBT**: These three packages are pure positioning-math libraries with zero React dependency (confirmed: no `createContext`/`forwardRef` in the output). Splitting them out of `react-vendor` reduces it by 19 KB (287 KB → 268 KB). The ES-module linker guarantees `floating-vendor` evaluates before `react-vendor` (since react-vendor statically imports @floating-ui/react which imports @floating-ui/core). `floating-vendor` added to `criticalScriptPatterns` at rank −1 so it arrives in the Link header before react-vendor. Both chunks download in parallel rather than sequentially. |
