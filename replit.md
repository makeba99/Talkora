...
79. Sesame CSM voice integration — AI Tutor's TTS now routes through a self-hosted Sesame CSM (Conversational Speech Model) inference server via `POST /api/ai-tutor/tts` (proxied by `server/sesame.ts`). A capability probe at `GET /api/ai-tutor/tts/health` decides at app load whether to use Sesame or the existing browser `speechSynthesis` engine — both wear the same `TtsCallbacks` contract (`createTts()` factory in `client/src/lib/ai-tutor/tts-factory.ts`). The Sesame engine (`SesameTtsEngine`) decodes generated WAV via Web Audio and drives mouth visemes from an `AnalyserNode` RMS amplitude loop (since CSM does not emit word-boundary events). CSM never runs inside Vextorn (Replit has no GPU) — it is deployed separately on Modal/RunPod/HF Endpoints/own GPU box using the reference FastAPI server in `csm-server/` (`server.py`, `requirements.txt`, `README.md`). Configuration is two optional env vars: `SESAME_CSM_URL` (required to enable) and `SESAME_CSM_TOKEN` (optional shared bearer). Same active-session gate as `/api/ai-tutor/chat` so other room participants cannot burn a user's GPU budget. If unset → silent fallback to browser TTS, no regression.
