...
73. Room-observable AI Tutor — one participant can speak with AI Tutor while other room participants see the LivePortrait-style animated SVG face, live caption, speaking state, and hear the same generated response through local speech synthesis. AI model routing prioritizes NVIDIA Nemotron when `NVIDIA_API_KEY` is configured, with concise voice-first replies that listen to the user's exact input.
